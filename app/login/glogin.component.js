"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var login_user_model_1 = require('./login.user.model');
var glogin_token_service_1 = require('./glogin.token.service');
//import { LocalStorageService } from 'angular-2-local-storage';
var GLoginComponent = (function () {
    function GLoginComponent(authService) {
        this.authService = authService;
        this.loginUser = new login_user_model_1.LoginUserModel('businessuser1', 'tester123');
        this.clientId = 'UMPB1q9XBKudOD58cyVYACOp22a5OjgY';
        this.token = '';
        this.error = "";
    }
    GLoginComponent.prototype.submitLogin = function (form) {
        this.authService.login(this.loginUser, this.clientId, '/dashboard');
        console.log(this.authService.isAuthenticated);
    };
    GLoginComponent.prototype.submitLogout = function () {
        this.authService.logout('/welcome');
    };
    GLoginComponent = __decorate([
        core_1.Component({
            selector: 'glogin',
            moduleId: module.id,
            templateUrl: './glogin.component.html'
        }), 
        __metadata('design:paramtypes', [glogin_token_service_1.GAuthenticationService])
    ], GLoginComponent);
    return GLoginComponent;
}());
exports.GLoginComponent = GLoginComponent;
//# sourceMappingURL=glogin.component.js.map